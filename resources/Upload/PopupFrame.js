...
function makeSimpleFrame(obj,url){
	...
	var abbotns = document.getElementById("abbotnsjs").value;
	var abbotcustom = document.getElementById('abbotcustomjs').value;
	var ssval = schsel.options[schsel.selectedIndex].value;
	//alert('NS '+abbotns+' CUSTOM '+abbotcustom);
	var pos = getPos(obj);
	var srcurl = url+"&conv="+ssval+"&abbotns="+abbotns+"&abbotcustom="+abbotcustom;
        var t = "<div class='innerpopup' style='width:400px;border:3px solid red;'>\n";
        t += "<iframe id='"+((new Date()).getTime())+"' width='99%' height='130px' style='border:0px;' src='"+srcurl+"'></iframe>";
	//alert(srcurl);
	//alert(t);
        t += "</div>";
        show(clickedobj,t);
